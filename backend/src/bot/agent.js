...
export async function runAgentChat(env, { prompt, userEmail, isAdmin, professionalContext, history = [] }) {
    ...
    // 📝 SELEÇÃO DE PROMPT
    let systemPrompt = "";
    if (role === 'master') systemPrompt = ADMIN_PROMPTS.system_master({ ...professionalContext, userEmail: userEmail });
    else if (role === 'owner') systemPrompt = ADMIN_PROMPTS.system_owner({ ...professionalContext, userEmail: userEmail });
    else if (role === 'staff') systemPrompt = ADMIN_PROMPTS.system_staff({ ...professionalContext, userEmail: userEmail });
